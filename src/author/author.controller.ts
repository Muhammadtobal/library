...
export class AuthorController {
  ...
  @Post()
  @UseInterceptors(
    FileInterceptor('image', uploadImage('authors', 'author')),
  )
 async create(@Body() createAuthorDto: CreateAuthorDto,@UploadedFile() file: Express.Multer.File,) {
  const data=await    this.authorService.create(createAuthorDto,file?.filename);
  return {
...
@Query() queryParams:any
 ...
 const {data,pagination}=await this.authorService.findAll(paginationQueryDto,filters)
 return {
  success: true,
 ...
 async findOne(@Param('id') id: number) {
  ...
  async update(@Param('id') id: number, @Body() updateAuthorDto: UpdateAuthorDto,@UploadedFile()file :Express.Multer.File) {
   const result= await this.authorService.update(id,updateAuthorDto,file?.filename)
   return {
    success: true,
