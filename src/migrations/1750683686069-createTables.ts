...
export class CreateTables1750683686069 implements MigrationInterface {
    name = 'CreateTables1750683686069'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`authors\` DROP COLUMN \`birth_date\``);
        await queryRunner.query(`ALTER TABLE \`authors\` DROP COLUMN \`death_date\``);
        await queryRunner.query(`ALTER TABLE \`authors\` DROP COLUMN \`nationality\``);
        await queryRunner.query(`ALTER TABLE \`authors\` DROP COLUMN \`genres\``);
        await queryRunner.query(`ALTER TABLE \`authors\` DROP COLUMN \`language\``);
        await queryRunner.query(`ALTER TABLE \`authors\` DROP COLUMN \`awards\``);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`books\` ADD CONSTRAINT \`FK_54f49efe2dd4d2850e736e9ab86\` FOREIGN KEY (\`authorId\`) REFERENCES \`authors\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`books\` ADD CONSTRAINT \`FK_a0f13454de3df36e337e01dbd55\` FOREIGN KEY (\`categoryId\`) REFERENCES \`categories\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`authors\` ADD \`awards\` text NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`authors\` ADD \`language\` varchar(50) NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`authors\` ADD \`genres\` text NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`authors\` ADD \`nationality\` varchar(100) NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`authors\` ADD \`death_date\` date NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`authors\` ADD \`birth_date\` date NULL DEFAULT 'NULL'`);
    }
